...
from .corr import CorrBlock, CorrBlock_fb, AlternateCorrBlock
...
class RAFT(nn.Module):
    def __init__(self, args):
        ...
        self.enc_dims = [32, 32, 64, 96] if args.small else [64, 64, 96, 128, 256]
        self.out_dim_f = 128 if args.small else 256
        # self.num_samples = self.out_dim_f if args.num_samples is None else args.num_samples
        ...
        if args.small:
            self.fnet = SmallEncoder(layers_dims=self.enc_dims, output_dim=self.out_dim_f, norm_fn='instance', dropout=args.dropout, pad_mode=args.pad_mode)
            self.cnet = SmallEncoder(layers_dims=self.enc_dims, output_dim=self.out_dim_c, norm_fn='none', dropout=args.dropout)
            self.update_block = SmallUpdateBlock(self.args, hidden_dim=hdim, out_dim=args.dim_out_flow)

        else:
            self.fnet = BasicEncoder(layers_dims=self.enc_dims, output_dim=self.out_dim_f, norm_fn='instance', dropout=args.dropout, pad_mode=args.pad_mode)
            self.cnet = BasicEncoder(layers_dims=self.enc_dims, output_dim=self.out_dim_c, norm_fn='instance', dropout=args.dropout)
            self.update_block = BasicUpdateBlock(self.args, hidden_dim=hdim, out_dim=args.dim_out_flow)
    ...
    def upsample_flow(self, flow, mask):
        ...
        up_flow = torch.einsum('bdn..., bdn...->bd...', mask, up_flow)
